...
import (
	...
	hellokitsvr "hello-goakit/gen/http/hello/kitserver"
	hellosvr "hello-goakit/gen/http/hello/server"
	"net/http"
	"net/url"
	"sync"
	"time"
...
func handleHTTPServer(ctx context.Context, u *url.URL, helloEndpoints *hello.Endpoints, wg *sync.WaitGroup, errc chan error, logger log.Logger, debug bool) {
	...
	// responses.
	var (
		helloHelloHandler *kithttp.Server
		helloServer       *hellosvr.Server
	)
	{
		eh := errorHandler(logger)
		helloHelloHandler = kithttp.NewServer(
			endpoint.Endpoint(helloEndpoints.Hello),
			hellokitsvr.DecodeHelloRequest(mux, dec),
			hellokitsvr.EncodeHelloResponse(enc),
		)
		helloServer = hellosvr.New(helloEndpoints, mux, dec, enc, eh, nil, nil)
	}

	// Configure the mux.
	hellokitsvr.MountHelloHandler(mux, helloHelloHandler)
	hellokitsvr.MountGenHTTPOpenapiJSON(mux)
